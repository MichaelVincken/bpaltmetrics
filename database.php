...
function select_person($firstname,$lastname,$con) {
    $query = mysqli_prepare($con,"SELECT pId FROM person WHERE name=? AND lastname=?");
    mysqli_stmt_bind_param($query, "ss", $firstname, $lastname);
    mysqli_stmt_execute($query);
    mysqli_stmt_bind_result($query,$pId);
    mysqli_stmt_fetch($query);
    
    mysqli_stmt_close($query);
    if($pId == null) {
        $query = mysqli_prepare($con,"INSERT INTO person(pId,name,lastname) VALUES (DEFAULT,?,?)");
        mysqli_stmt_bind_param($query, "ss", $firstname, $lastname);
        mysqli_stmt_execute($query);
        mysqli_stmt_close($query);
...
$result = mysqli_query($con,$query) or die('Cannot get url. '.mysqli_error($con));
$resultarray = array();
while($row = mysqli_fetch_array($result)) {
    array_push($resultarray,$row);
}
